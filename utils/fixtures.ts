...
// Same pattern — any test declaring 'testimonialsPage' gets this injected automatically
  ...
  bannersPage: async ({ page }, use) => {
    const bannersPage = new BannersPage(page);
    await use(bannersPage);
  },
  ...
  videoAssetPage: async ({ page }, use) => {
    const videoAssetPage = new VideoAssetPage(page);
    await use(videoAssetPage);
  }
